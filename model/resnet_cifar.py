...
def resnet20(pretrained=True, checkpoint=None):
...
def resnet56(pretrained=True, checkpoint=None):
    model = ResNet(BasicBlock, [9, 9, 9])
    if pretrained:
        model.load_state_dict(torch.load(checkpoint))
    return model

def resnet56_cifar100(pretrained=True, checkpoint=None):
    model = ResNet(BasicBlock, [9, 9, 9], num_classes=100)
    if pretrained:
        model.load_state_dict(torch.load(checkpoint))
    return model
